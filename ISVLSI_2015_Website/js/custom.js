...
$(document).ready(function () {
	...
	// Countdown //
	$('#countdown').countdown({
		until: new Date(2015, 6 - 1, 15), // new Date(year, mth - 1, day, hr, min, sec) - date/time to count down to 
		// or numeric for seconds offset, or string for unit offset(s): 
		// 'Y' years, 'O' months, 'W' weeks, 'D' days, 'H' hours, 'M' minutes, 'S' seconds 
		// until: '-1m +1d', for demo
		timezone: -0, // The timezone (hours or minutes from GMT) for the target times, or null for client local 
	...
	function liftOff() {
		...
		$('.register-title').text('Late Registration Active');//..addClass('hidden');
		//$('.register-box').append('<h2>Early Registration is Now Over!</h2>');
	...
	// Google Map //
	$('#map_canvas').gmap({
		'center': new google.maps.LatLng(43.612946, 3.873548), // Change this to your desired latitude and longitude
		'zoom': 17,
		 'scrollwheel': false,
	    'scaleControl': false,
	    'draggable': false,
	...
	$('#map_canvas').gmap().bind('init', function () {
		$('#map_canvas').gmap('addMarker', {
			'id': 'marker-1',
			'position': '43.612946, 3.873548',
			'bounds': false,
			'icon': image
		}).click(function () {
			$('#map_canvas').gmap('openInfoWindow', {
			    'content': '<h4>ISVLSI 2015</h4><p><strong>Faculte de Medecine</strong><br>2 Rue de lEcole de Medecine<br>34000 Montpellier<br>France<br></p>'
//<strong>Faculte de Medecine</strong><br>
//2 Rue de lÉcole de Médecine<br>
